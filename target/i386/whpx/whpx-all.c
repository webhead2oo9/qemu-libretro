 ...
 * hardware would do for the access the emulator is about to perform.
 *
 * This matters for pageable guests doing device I/O into pageable memory
                                              ...
                                              MMUTranslateFlags flags)
{
    ...
    if (flags & MMU_TRANSLATE_VALIDATE_WRITE) {
        wflags = WHvTranslateGvaFlagValidateWrite;
    } else if (flags & MMU_TRANSLATE_VALIDATE_EXECUTE) {
        wflags = WHvTranslateGvaFlagValidateExecute;
    } else {
        wflags = WHvTranslateGvaFlagValidateRead;
    }
    if (flags & MMU_TRANSLATE_PRIV_CHECKS_EXEMPT) {
        wflags |= WHvTranslateGvaFlagPrivilegeExempt;
    }
    wflags |= WHvTranslateGvaFlagSetPageTableBits;

    hr = whp_dispatch.WHvTranslateGva(whpx->partition, cpu->cpu_index,
                                      gva, wflags, &res, &res_gpa);
    if (SUCCEEDED(hr)) {
        ...
        default:
            /*
             * E.g. GpaUnmapped for MMIO destinations: the hypervisor does
             * not hand back the GPA, so take the software walk instead.
             */
            break;
        }
    }
...
static const struct x86_emul_ops whpx_x86_emul_ops = {
    .mmu_gva_to_gpa = whpx_mmu_gva_to_gpa,
    ...
    do {
        ...
        case WHvRunVpExitReasonX64MsrAccess: {
            ...
            if (!whpx_irqchip_in_kernel() &&
                ...
                if (!vcpu->exit_ctx.MsrAccess.AccessInfo.IsWrite) {
                    bql_lock();
                    /* 9.0 base: apic_msr_read() resolves the current APIC itself */
                    msr_ret = apic_msr_read(index, &val);
                    bql_unlock();
                    ...
                    bql_lock();
                    msr_ret = apic_msr_write(index, val);
                    bql_unlock();
                    if (msr_ret < 0) {
                        x86_emul_raise_exception(&X86_CPU(cpu)->env, EXCP0D_GPF, 0);
                        raises_gpf = true;
...
int whpx_init_vcpu(CPUState *cpu)
{
    ...
    if (!whpx_irqchip_in_kernel() && x86_cpu->apic_state != NULL) {
        /* 9.0 base: apic_state is a DeviceState, cast to reach the APIC fields */
        WHV_REGISTER_VALUE apic_id = {
            .Reg64 = APIC_COMMON(x86_cpu->apic_state)->initial_apic_id
        };
...
void whpx_cpu_instance_init(CPUState *cs)
{
    ...
    /* 9.0 base: max_features lives on the instance, not the class */
    if (cpu->max_features) {
